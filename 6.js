...
class TwoStepProcessor extends AuthProcessor {
  ...
  validate(username, passkey) {
    if(username === "jonh" && passkey === "password" && this.isValidTwoStepCode()) {
...
class RoleProcessor extends AuthProcessor {
  ...
  validate() {
    if(role === "guest") {
      ...
      return this.nextProcessor.validate(username, passkey, role);
    } else {
      return false;
  }
}
...
class CredentialsProcessor extends AuthProcessor {
  ...
  validate() {
    if(username === "admin" && passkey === "admin123") {
      ...
      return super.validate(username, passkey, role);
    }
  }
}
